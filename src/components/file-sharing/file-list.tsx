...
export default function FileList() {
  ...
  const deleteRecord = async (id: number) => {
    ...
    if (error) console.error("Error deleting record:", error);
      ...
      setFile((prev) => prev.filter((r) => r.id !== id));
    }
  };
  ...
  return (
    <div className="space-y-3 py-10">
      ...
      {file.length === 0 && !loading && (
        <div className="flex items-center justify-center gap-2 rounded-xl border px-4 py-2">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button size={"icon"} asChild variant={"ghost"}>
                        <Link
                          target="_blank"
                          href={`${process.env.NEXT_PUBLIC_CLIENT_URL}/f/${f.short_code}`}
                    ...
                    <TooltipContent className="px-2 py-1 text-xs">
                      {`${process.env.NEXT_PUBLIC_CLIENT_URL}/f/${f.short_code}`}
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
